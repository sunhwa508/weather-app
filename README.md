...
**문제의 코드 발견
이 코드의 문제점. 지금과 같이 간단히 10개 이하의 코드라면 ,, 몇초차이나지 않을것이다.<br/>
...
## ✔Acknowledgments
처음으로 제대로된 코드 리뷰를 받고 (스터디,개발Slack 등에서 도움을 받음) 앞으로의 공부 방향성을 잡게되어 너무 기분이 좋다 😁😁<br/>
항상 공부하면서 느꼇던 점인 "내가 만든 프로젝트들은" 전부 모양, 색만 다를뿐 결국 같은 코드의 반복이었고 그 이상의 발전이없었다는 점이었다.<br/>
다행히도 실무에 계신 많은 개발자분들의 도움으로 나의 문제점을 파악할 수 있었다. 기능만을 구현하는 코드가 아닌, <br/>
추후 한번 쓰고 버려질 코드는 좋은 코드가 아니라는 사실, 기능구현에만 치중 하지 않고, 형상관리, 협업(팀개발), 예외처리 디버그 등 앞에서 보이는것이 다가 아닌 10년 20년 장수할 코드는 만드는 것이 나의 다음 목표가 되겠다 :) 
도움을 주신 코드리뷰에 도움을주신 물개님/doe님께 감사드립니다 ^^
