...
## ✔Acknowledgments
처음으로 제대로된 코드 리뷰를 받고 (스터디,개발Slack 등에서 도움을 받음) 앞으로의 공부 방향성을 잡게되어 너무 기분이 좋았다<br/>
항상 공부하면서 느꼇던 모양, 색만 다를뿐 결국 같은 코드의 반복이었고 그 이상의 발전이없었다는 점<br/>
다행히도 실무에서 뛰고계신 많은 개발자분들의 도움으로 나의 문제점을 파악할 수 있었으며, 기능만을 구현하는 코드가 아닌, <br/>
추후 한번쓰고 버려질 코드가 되지 않는 법을 공부 할 수 있는 기회가 되었고 앞으로도 기술에만 치중 하지 않고, 형상관리, 협업(팀개발), 예외처리 디버그 등 앞에서 보이는것이 다가 아닌 10년 20년 장수할 코드는 만드는 것이 나의 다음 목표가 되겠다 :) 
